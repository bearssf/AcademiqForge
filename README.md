...
- **The Anvil** (`/app/project/:id/anvil`): **Quill** rich-text drafts (HTML in `project_sections.body`); plain-text drafts are migrated to paragraphs on load. Autosave; **section-switch guard** (citation heuristic + modal before changing sections); **split rail** — **score strip**, **feedback & suggestions** (Bedrock when configured, Apply/Ignore), **citations** with **Insert citation**; **export** — [Anvil vision](docs/anvil-vision.md). **AWS Bedrock:** [docs/aws-bedrock.md](docs/aws-bedrock.md).
