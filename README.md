...
- **The Anvil** (`/app/project/:id/anvil`): per-section draft editor with autosave; drafts persist in `project_sections.body`. Right column: **split rail** — citations (bottom) lists **Crucible sources linked to the active section**; feedback (top) still placeholder — see [Anvil vision](docs/anvil-vision.md).
