...
| POST | `/api/projects/:id/anvil/upload` | Multipart field **`image`** — upload an image for the Anvil editor (max **5 MB**, image types only). Returns `{ "url" }` path under `/uploads/anvil/`. Requires project ownership. |
