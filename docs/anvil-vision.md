...
| **6** | **Feedback persistence** | *(Shipped.)* Table **`anvil_suggestions`** (`suggestion_status` in SQL, **`status`** in JSON). API: `GET/POST /api/projects/:id/sections/:sectionId/suggestions`, `PATCH /api/projects/:id/suggestions/:suggestionId` (`status`: `applied` \| `ignored`). Rail lists suggestions; **Apply** / **Ignore** for `open` rows. **Optional:** `ANVIL_DRAFT_STALE_FEEDBACK=1` — each section has **`draft_revision`**; new suggestions store **`draft_revision_at_generation`**. Saving the section body bumps the revision and **removes open** rows tied to an older revision; **applied/ignored** stay, with an **Earlier draft** hint when the draft has moved on. Disable by unsetting the env (rollback). | `lib/schema.js`, `lib/anvilFeedback.js`, `lib/anvilStaleFeedback.js`, `routes/api.js`, `anvil.js`, `app-anvil-rail.ejs` |
