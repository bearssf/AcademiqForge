...
| **10** | **Progress awareness (center)** | *(Shipped.)* **Bar** under the section title: **word count** (plain text from editor), **section** status + **progress %** from bundle, **Last AI review** relative time from last successful Bedrock review round (`lastReviewAt`). Updates on type/save/review; 60s tick for relative labels. | `anvil.js`, `app-shell.css` |
