...
function normalizeTemplateSectionPercents(sections) {
  ...
  const parsed = sections.map((s) => {
    ...
    const p = parseInt(String(s.percent), 10);
    return Number.isNaN(p) ? null : Math.min(100, Math.max(0, p));
  });
...
function validateTemplatesForSave(tpl) {
  ...
  for (const k of keys) {
    ...
    const rawPcts = t.sections.map((s) => {
      ...
      const p = parseInt(String(s.percent), 10);
      return Number.isNaN(p) ? NaN : Math.min(100, Math.max(0, p));
    });
    ...
    if (hasAll) {
      ...
      if (sum !== 100) {
        return {
          ok: false,
          error: `Template "${k}": section percentages must total 100% (currently ${sum}%).`,
        };
      }
    }
    if (t.projectedTotalWords != null && t.projectedTotalWords !== '') {
