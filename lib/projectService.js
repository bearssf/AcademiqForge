...
const { PURPOSE_KEYS_ORDERED, normalizePurposeToKey } = require('./canonicalSelects');

/** Canonical purpose keys; labels from locales (projectPurposes.*). */
const PURPOSES = PURPOSE_KEYS_ORDERED;
...
function loadTemplatesFromFileSync() {
  const raw = fs.readFileSync(TEMPLATES_FILE, 'utf8');
  return JSON.parse(raw);
}

/**
 * In-memory templates. After `initProjectTemplatesStore`, this is loaded from MySQL so deploys
 * cannot overwrite production templates via `data/project-templates.json` in the repo.
 */
function loadTemplates() {
  if (templatesCache) return templatesCache;
  templatesCache = loadTemplatesFromFileSync();
...
/**
 * Call once after DB schema exists. Prefers DB row; if missing, seeds from `data/project-templates.json`
 * (repo defaults) and persists to DB.
 */
async function initProjectTemplatesStore(getPool) {
  const r = await query(getPool, 'SELECT json_body FROM project_templates_store WHERE id = 1');
  const row = r.recordset && r.recordset[0];
  if (row && row.json_body) {
    try {
      templatesCache = JSON.parse(String(row.json_body));
      console.log(
        'Project templates: loaded from database (' + Object.keys(templatesCache || {}).length + ' keys).'
      );
      return;
    } catch (e) {
      console.error('Project templates: invalid JSON in database, falling back to file:', e.message);
    }
  }
  const fromFile = loadTemplatesFromFileSync();
  const payload = `${JSON.stringify(fromFile, null, 2)}\n`;
  await query(getPool, 'INSERT INTO project_templates_store (id, json_body) VALUES (1, @j)', {
    j: payload.trim(),
  });
  templatesCache = fromFile;
  console.log(
    'Project templates: seeded database from data/project-templates.json (' +
      Object.keys(fromFile).length +
      ' keys). Edits are stored in MySQL and survive deploys.'
  );
}
...
function validateTemplatesForSave(tpl) {
  ...
  for (const k of keys) {
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      if (s.projectedWords != null && s.projectedWords !== '') {
        const pw = Number(s.projectedWords);
        if (!Number.isFinite(pw) || pw < 0 || pw > 500000) {
          return {
            ok: false,
            error: `Template "${k}": section ${i + 1} word target must be between 0 and 500000.`,
          };
        }
      }
    }
...
/**
 * Persist templates to MySQL (survives deploys; repo file on disk is no longer the source of truth).
 * Optionally mirrors to `data/project-templates.json` when PROJECT_TEMPLATES_MIRROR_FILE=1 (local dev).
 * @returns {Promise<{ ok: true } | { ok: false, error: string }>}
 */
async function saveProjectTemplates(getPool, tpl) {
  ...
  try {
    await query(
      getPool,
      `INSERT INTO project_templates_store (id, json_body) VALUES (1, @j)
       ON DUPLICATE KEY UPDATE json_body = VALUES(json_body), updated_at = NOW(6)`,
      { j: payload.trim() }
    );
    templatesCache = tpl;
    const mirror = String(process.env.PROJECT_TEMPLATES_MIRROR_FILE || '')
      .trim()
      .toLowerCase();
    ...
    return { ok: false, error: e.message || 'Could not save templates.' };
  }
}
...
function attachTemplateMeta(bundle) {
  ...
  const sectionWordTargetsBySlug = {};
  for (const s of def.sections || []) {
    if (!s || s.slug == null) continue;
    const slug = String(s.slug).trim();
    if (!slug) continue;
    if (s.projectedWords != null && s.projectedWords !== '') {
      const pw = Math.round(Number(s.projectedWords));
      if (Number.isFinite(pw) && pw >= 0 && pw <= 500000) {
        sectionWordTargetsBySlug[slug] = pw;
      }
    }
  }
  bundle.templateMeta = {
...
async function listProjects(getPool, userId) {
  ...
  return r.recordset.map((row) => ({
    ...row,
    purpose: normalizePurposeToKey(row.purpose) || row.purpose,
  }));
}
...
async function getProjectBundle(getPool, projectId, userId) {
  ...
  const projRow = { ...proj.recordset[0] };
  projRow.purpose = normalizePurposeToKey(projRow.purpose) || projRow.purpose;
  if (String(projRow.status || '').toLowerCase() === 'canceled') {
    return null;
  }
 ...
 */
/**
 * Soft-cancel: marks project as canceled (does not delete rows).
 * @returns {Promise<{ ok: true } | { ok: false, status: number, error: string }>}
 */
async function cancelProject(getPool, userId, projectId) {
  const r = await query(
    getPool,
    `UPDATE projects SET status = 'canceled', updated_at = NOW() WHERE id = @id AND user_id = @user_id`,
    { id: projectId, user_id: userId }
  );
  if (!r.rowsAffected[0]) return { ok: false, status: 404, error: 'Not found' };
  return { ok: true };
}
