...
async function insertAnvilSuggestions(getPool, projectId, sectionId, items) {
  ...
  if (staleFeedbackEnabled()) {
    const revRes = await p
      .request()
      .input('section_id', sql.Int, sectionId)
      .query(`SELECT draft_revision FROM project_sections WHERE id = @section_id`);
    revAtInsert =
  ...
  for (const raw of items) {
    ...
    let ins;
    if (staleFeedbackEnabled()) {
      req.input('draft_rev_gen', sql.Int, revAtInsert);
      ins = await req.query(`
        INSERT INTO anvil_suggestions (project_id, section_id, category, body, anchor_json, draft_revision_at_generation, updated_at)
        OUTPUT INSERTED.id
        VALUES (@project_id, @section_id, @category, @body, @anchor_json, @draft_rev_gen, GETDATE())
      `);
    } else {
      ins = await req.query(`
