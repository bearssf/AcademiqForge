...
async function ensureCoreSchema(getPool) {
  ...
  await p.request().query(`
    ...
    AND EXISTS (SELECT 1 FROM projects WHERE id = 8)
    AND NOT EXISTS (SELECT 1 FROM research_plan_items WHERE project_id = 8)
    BEGIN
      INSERT INTO research_plan_items (project_id, section_id, section_title, suggestion_body, keywords, research_needed, status)
      VALUES (8, 0, 'Introduction',
        'The claim that critical thinking is impactful to intercultural relationships needs evidence to support',
        'Intercultural relationships, Critical Thinking',
        'Evidence/Citation', 'unresolved');
      INSERT INTO research_plan_items (project_id, section_id, section_title, suggestion_body, keywords, research_needed, status)
      VALUES (8, 0, 'Introduction',
        'The claim that critical thinking is impactful to intercultural relationships needs evidence to support',
        'Intercultural relationships, Critical Thinking',
