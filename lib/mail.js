...
async function sendMail(opts) {
  ...
  const secure =
    ...
    port === 465;
  const tlsExplicit = String(process.env.SMTP_USE_TLS || '').trim().toLowerCase();
  const useTls =
    !secure &&
    tlsExplicit !== '0' &&
    tlsExplicit !== 'false' &&
 ...
 */
async function sendRegistrationVerificationEmail(opts) {
  const appName = 'AcademiqForge';
  const subject = `Your ${appName} verification code`;
  const text = [
    `Your ${appName} verification code is: ${opts.code}`,
    '',
    `This code expires in ${REGISTRATION_CODE_EXPIRY_MINUTES} minutes.`,
    '',
    `If you did not sign up for ${appName}, you can ignore this email.`,
  ].join('\n');
  const html = `<p>Your <strong>${appName}</strong> verification code is:</p>
<p style="font-size:1.5rem;letter-spacing:0.15em;font-weight:600;">${opts.code}</p>
<p style="color:#666;font-size:13px;">This code expires in ${REGISTRATION_CODE_EXPIRY_MINUTES} minutes.</p>
<p style="color:#666;font-size:13px;">If you did not sign up for ${appName}, you can ignore this email.</p>`;
  return sendMail({ to: opts.to, subject, text, html });
}
