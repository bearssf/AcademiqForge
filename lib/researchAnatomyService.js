...
async function insertRun(getPool, { userId, projectId, s3Key, contentVersion, status }) {
  const ins = await query(
    getPool,
    `INSERT INTO research_anatomy_runs (user_id, project_id, s3_key, content_version, status)
     VALUES (@uid, @pid, @s3, @ver, @st)`,
    {
      uid: userId,
      pid: projectId,
      s3: s3Key || null,
...
async function updateRunComplete(getPool, runId, resultsObj) {
...
async function processResearchAnatomyRun(getPool, runId, userId, projectId, opts) {
  ...
  try {
    ...
    await updateRunComplete(getPool, runId, evaluation);
  } catch (e) {
