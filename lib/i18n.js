...
function loadBundles() {
  ...
  for (const code of SUPPORTED_LOCALES) {
    ...
    if (fs.existsSync(p)) {
      ...
      bundles[code] = deepMerge({}, base, overlay);
    } else {
      bundles[code] = base;
    }
  }
...
function localeOptionsForSelect() {
  return SUPPORTED_LOCALES.map((id) => ({ id, label: LOCALE_LABELS[id] || id }));
}
...
function attachLocaleMiddleware() {
  ...
  return function attachLocale(req, res, next) {
    ...
    res.locals.localeOptions = localeOptionsForSelect();
