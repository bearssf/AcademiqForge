...
function resolveBedrockModelId() {
  const profile =
    process.env.BEDROCK_INFERENCE_PROFILE_ARN && String(process.env.BEDROCK_INFERENCE_PROFILE_ARN).trim();
  if (profile) return profile;
  const modelId = process.env.BEDROCK_MODEL_ID && String(process.env.BEDROCK_MODEL_ID).trim();
  return modelId || '';
}

function isBedrockConfigured() {
  const region = process.env.AWS_REGION && String(process.env.AWS_REGION).trim();
