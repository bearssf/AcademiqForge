...
async function buildSectionDocxBuffer({ title, html, citationStyle, sectionSlug }) {
  ...
  const children = [
    new Paragraph({
      ...
      children: [new TextRun({ text: heading, bold: true, size: 32, ...trOpts })],
    }),
    ...bodyChildren,
  ];
  const doc = new Document({
...
async function buildProjectDocxBuffer({ projectName, sections, citationStyle }) {
  ...
  const flat = [
    new Paragraph({
      spacing: lineSpacingForCitationStyle(cs),
      children: [new TextRun({ text: title, bold: true, size: 32, ...trOpts })],
    }),
  ];
  for (let i = 0; i < (sections || []).length; i++) {
    ...
    flat.push(
      new Paragraph({
        spacing: lineSpacingForCitationStyle(cs),
        children: [
          new TextRun({ text: String(sec.title || 'Section'), bold: true, size: 28, ...trOpts }),
        ],
      })
    );
